 ...
 * <em>world-space</em>: the {@link Renderer} applies the active
 * {@link Camera} transform before executing their draw commands, so
...
public enum RenderLayer {
    /**
     * Furthest back. Intended for sky gradients, solid colour fills, and any
     * static full-screen backdrop that should sit behind all other content.
     * <p>World-space — camera transform applied.</p>
     */
    BACKGROUND("Background", true),
 
    /**
     * Tile maps, terrain details, and ground-level scenery that entities walk
     * on top of.
     * <p>World-space — camera transform applied.</p>
     */
    WORLD("World", true),
    ...
    /**
     * All active game entities: players, enemies, NPCs, projectiles, pickups,
     ...
     * flashes, and short-lived animations. Drawn after {@link #ENTITIES} so
     * effects appear in front of the objects that spawned them.
     * <p>World-space — camera transform applied.</p>
     */
    FX("FX", true),
 
    /**
     * Screen-space HUD and menus: health bars, score, inventory, dialogue
     * boxes, pause overlays. Coordinates are in screen pixels regardless of
     * camera position or zoom.
     * <p>Screen-space — camera transform NOT applied.</p>
     */
    UI("UI", false),
 
    /**
     * Drawn last, always on top. Collision shapes, entity origins, FPS
     * counter, mouse coordinates, and any other diagnostic overlay.
     * Gated by {@link com.lobsterchops.deepclaw.engine.rendering.debug.DebugRenderer#isEnabled()};
    ...
    /**
     * Whether the {@link Renderer} should apply the active {@link Camera}
     * transform before executing draw commands on this layer.
     * <p>
    ...
    /**
     * @return Human-readable layer name, e.g. {@code "Entities"}.
    ...
    /**
     * @return {@code true} if the {@link Renderer} should apply the
     *         {@link Camera} transform before drawing on this layer.
     *         {@code false} for screen-space layers ({@link #UI},
     *         {@link #DEBUG}).
     */
    ...
    /**
     * @return {@code true} if this layer is rendered in screen space
     *         (i.e. the camera transform is <em>not</em> applied).
     *         Convenience inverse of {@link #isWorldSpace()}.
     */
    ...
    /**
     * Returns the layer immediately below this one in draw order, or
     * {@code null} if this is {@link #BACKGROUND} (already the first layer).
     *
     * <p>Useful for debug tooling that wants to walk the layer stack.</p>
     *
    ...
    /**
     * Returns the layer immediately above this one in draw order, or
     * {@code null} if this is {@link #DEBUG} (already the last layer).
     *
     * <p>Useful for debug tooling that wants to walk the layer stack.</p>
     *
