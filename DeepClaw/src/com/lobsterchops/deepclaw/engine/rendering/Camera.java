 ...
 *
 * <p>
 * {@code Camera} owns the mapping between <em>world space</em> (game units) and
 * <em>screen space</em> (pixels on
 * {@link com.lobsterchops.deepclaw.engine.core.GamePanel}). The
 * {@link Renderer} retrieves the camera's {@link AffineTransform} once per
 ...
 * <em>top-left corner</em> of the viewport — i.e. what the camera is looking at
 * in the upper-left of the screen.</li>
 * </ul>
 *
 ...
 * </pre>
 * <p>
 * Zoom is applied around the top-left of the viewport (which is sufficient for
 * most game-jam use cases). If you want zoom centred on a world point, combine
 * {@link #centerOn(float, float)} with the zoom adjustment in the same tick so
 * the view centre stays fixed.
 * </p>
 *
 * <h3>Ownership</h3>
 * <p>
...
public final class Camera {
	...
	/**
	 * World-space X coordinate of the viewport's top-left corner. Increasing this
	 * value scrolls the view to the right.
	 */
	private float x;

	/**
	 * World-space Y coordinate of the viewport's top-left corner. Increasing this
	 * value scrolls the view downward.
	 */
	private float y;
	...
	/**
	 * Width of the viewport in screen pixels. Updated when the panel is resized
	 * (not yet wired — stored for future use).
	 */
	...
	/**
	 * Returns the {@link AffineTransform} the {@link Renderer} should apply before
	 * executing world-space draw commands.
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * A new {@code AffineTransform} instance is returned on every call — do not
	 * cache it across frames.
	 * </p>
	 *
	 * @return A fresh {@link AffineTransform} representing the current camera
	 *         state.
	 */
	 ...
	 *
	 * <p>
	 * Inverse of the camera transform's X component. Use this to find what world
	 * position the player's cursor is pointing at.
	 * </p>
	 *
	 * <pre>
	 ...
	 *
	 * <p>
	 * Use this to check whether a world object is on-screen before submitting its
	 * draw command (manual culling). For automatic culling, prefer
	 ...
	 *
	 * <p>
	 * The check is done in world space — no pixel rounding is applied, which keeps
	 * it fast and avoids off-by-one pops at the viewport edge.
	 * </p>
	 *
	...
	public boolean isVisible(float wx, float wy, int w, int h) {
		...
		return wx + w > x && wy + h > y && wx < x + viewWorldWidth && wy < y + viewWorldHeight;
	}

	/**
	 * Positions the camera so that the given world point appears at the centre of
	 * the viewport.
	 *
	 * <p>
	...
	public void setViewSize(int viewWidth, int viewHeight) {
		...
		if (viewHeight <= 0)
			throw new IllegalArgumentException("viewHeight must be > 0.");
	...
	public String toString() {
		return "Camera{x=" + x + ", y=" + y + ", zoom=" + zoom + ", view=" + viewWidth + "x" + viewHeight + '}';
	}
}