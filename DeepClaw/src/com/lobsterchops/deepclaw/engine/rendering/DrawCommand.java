 ...
 * {@code setTransform}, {@code setClip}, …) carries over to subsequent
 * commands in the same layer unless explicitly restored.
 * </p>
 * <p>
 * The convention is: <strong>restore any state you change.</strong> Use
 * {@link Graphics2D#create()} to get a disposable copy if your command needs
 * to mutate state without affecting neighbours:
 * </p>
 *
 * <pre>
 ...
 * executing commands on world-space layers ({@link RenderLayer#isWorldSpace()}).
 * Commands submitted to screen-space layers ({@link RenderLayer#UI},
 * {@link RenderLayer#DEBUG}) receive a plain, untransformed context, so
...
public interface DrawCommand {
     ...
     *
     * <p>
     * Called once per frame by {@link Renderer#flush()} for each queued command,
     * in layer order. The context is the live back-buffer — do not store or cache
     * it beyond this call.
     * </p>
     *
    ...
    static DrawCommand noop() {
        return g -> { /* intentional no-op */ };
    }
 
    /**
     * Composes two commands into one, executing {@code first} before
     * {@code second} on the same {@link Graphics2D} context.
     *
     * <p>
     * Handy when a single logical element needs two separate draw passes
     * (e.g. a shadow below and a sprite above) but you want to submit them
     * as a single queue entry:
     * </p>
     *
     * <pre>
